...
export default function PublisherAuthorWiseSales() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await api.get('/auth/reports/publisher-author-wise-sales/', {
        params: {
          branch_id: branchId,
  ...
  const handleExportPDF = () => {
    ...
    printWindow.document.write(`
      ...
      <html>
      <head>
        <title>Publisher Author Wise Sales Report</title>
        <style>
          * { box-sizing: border-box; }
          ...
          .publisher-header { font-weight: bold; }
          .publisher-total td, .grand-total td {
            font-weight: bold;
            border-top: 1px solid #b5b3b3;
            border-bottom: 1px solid #b5b3b3;
            padding: 5px 6px;
  ...
  const handleExportExcel = () => {
    ...
    // Add header row
    excelData.push(['Publisher', 'Author', 'Title', 'Quantity', 'Value']);
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Publisher Author Sales');

    // Generate filename with date range
    const filename = `Publisher_Author_Wise_Sales_${formData.date_from}_to_${formData.date_to}.xlsx`;
  ...
  // Render the report table matching JasperReports format with publisher grouping
  const renderReport = () => {
    ...
    const groupedData = {};
    reportData.forEach(row => {
      const publisher = row.publisher_nm || 'Unknown Publisher';
      if (!groupedData[publisher]) {
        groupedData[publisher] = [];
      }
      groupedData[publisher].push(row);
    });
    ...
    return (
      <div ref={reportRef} className="bg-white">
        ...
        <div className="report-header mb-4">
          ...
          <div className="report-title text-base font-bold mt-2">Sale Register (Publisher / Author)</div>
        ...
        <table className="w-full text-xs">
          <thead>
            <tr className="border-t border-b border-gray-300">
              <th className="py-2 px-1 text-left w-40">Author</th>
              <th className="py-2 px-1 text-left">Title</th>
              <th className="py-2 px-1 text-right w-16">Qty</th>
              <th className="py-2 px-1 text-right w-20">Value</th>
            </tr>
          </thead>
          <tbody>
            {Object.entries(groupedData).map(([publisher, rows]) => {
              let publisherQtyTotal = 0;
              let publisherValueTotal = 0;

              return (
                <React.Fragment key={publisher}>
                  {/* Publisher Header */}
                  <tr className="publisher-header bg-gray-50">
                    <td colSpan={4} className="py-2 px-1 font-bold">{publisher}</td>
                  </tr>

                  {/* Publisher Items */}
                  {rows.map((row, index) => {
                    publisherQtyTotal += row.quantity || 0;
                    publisherValueTotal += row.value || 0;
                    grandQtyTotal += row.quantity || 0;
                    grandValueTotal += row.value || 0;

                    return (
                      <tr key={`${publisher}-${index}`} className="hover:bg-gray-50">
                  ...
                  {/* Publisher Total Row */}
                  <tr className="publisher-total border-t border-b border-gray-300">
                    <td colSpan={2} className="py-2 px-1 font-bold">Publisher Total</td>
                    <td className="py-2 px-1 text-right font-bold">{formatInteger(publisherQtyTotal)}</td>
                    <td className="py-2 px-1 text-right font-bold">{formatNumber(publisherValueTotal)}</td>
                  </tr>
                </React.Fragment>
              );
            })}
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 md:p-6">
      ...
      <PageHeader
        ...
        title="Publisher Author Wise Sales"
        subtitle="Generate publisher and author wise sales report"
      />
