...
export default function SaleClassRatio() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await api.get('/auth/reports/sale-class-ratio/', {
        params: {
          branch_id: branchId,
  ...
  const handleExportPDF = () => {
    ...
    printWindow.document.write(`
      ...
      <html>
      <head>
        <title>Sales Class Ratio Report</title>
        <style>
  ...
  const handleExportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Sales Class Ratio');

    // Generate filename with date range
    const filename = `Sales_Class_Ratio_${formData.date_from}_to_${formData.date_to}.xlsx`;
    ...
    return (
      <div ref={reportRef} className="bg-white">
        ...
        <div className="report-header mb-4">
          ...
          <div className="report-title text-base font-bold mt-2">Sales Class Ratio</div>
        ...
        <table className="w-full text-xs">
          <thead>
            <tr className="border-t border-b border-gray-300">
              <th className="py-2 px-1 text-left">Class</th>
          ...
          <tbody>
            {reportData.map((row, index) => {
              ...
              return (
                <tr key={index} className="hover:bg-gray-50">
                  <td className="py-1 px-1 font-serif">{row.class_nm || ''}</td>
                  <td className="py-1 px-1 text-right">{formatNumber(row.amount)}</td>
                </tr>
              );
            })}
            ...
            <tr className="grand-total border-t-2 border-b-2 border-gray-400">
              ...
              <td className="py-2 px-1 text-right font-bold text-sm">{formatNumber(grandTotal)}</td>
            </tr>
          </tbody>
        </table>
      </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 md:p-6">
      ...
      <PageHeader
        ...
        title="Sales Class Ratio"
        subtitle="Generate sales class ratio report"
      />
