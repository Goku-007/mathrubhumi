...
export default function CategoryPublisherAuthorWiseSales() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await api.get('/auth/reports/category-publisher-author-wise-sales/', {
                params: {
                    branch_id: branchId,
    ...
    const handleExportPDF = () => {
      ...
      <html>
      <head>
        <title>Category Publisher Author Wise Sales</title>
        <style>
          * { box-sizing: border-box; }
          ...
          .category-total td, .grand-total td {
            font-weight: bold;
            border-top: 1px solid #b5b3b3;
            border-bottom: 1px solid #b5b3b3;
            padding: 5px 6px;
    ...
    const handleExportExcel = () => {
        ...
        // Add header row
        excelData.push(['Category', 'Publisher', 'Author', 'Title', 'Quantity', 'Value']);
        ...
        reportData.forEach(row => {
            ...
            grandValueTotal += row.value || 0;
        });

        // Add grand total
        excelData.push(['Grand Total', '', '', '', grandQtyTotal, grandValueTotal]);
        ...
        XLSX.utils.book_append_sheet(wb, ws, 'Category Publisher Author');

        // Generate filename with date range
        const filename = `Category_Publisher_Author_Wise_Sales_${formData.date_from}_to_${formData.date_to}.xlsx`;
    ...
    // Render the report table matching JasperReports format with category grouping
    const renderReport = () => {
        ...
        const groupedData = {};
        reportData.forEach(row => {
            const category = row.sub_category_nm || 'Unknown Category';
        ...
        return (
            <div ref={reportRef} className="bg-white">
                ...
                <div className="report-header mb-4">
                    ...
                    <div className="report-title text-base font-bold mt-2">Sale Register (Category / Publisher / Author)</div>
                    <div className="report-subtitle text-sm text-gray-600">
                        Sales from {formatDate(reportParams?.date_from)} to {formatDate(reportParams?.date_to)}
                    </div>
                </div>
                ...
                <table className="w-full text-xs">
                    <thead>
                        <tr className="border-t border-b border-gray-300">
                            <th className="py-2 px-1 text-left w-28">Publisher</th>
                            <th className="py-2 px-1 text-left w-28">Author</th>
                            <th className="py-2 px-1 text-left">Title</th>
                            <th className="py-2 px-1 text-right w-16">Qty</th>
                            <th className="py-2 px-1 text-right w-24">Value</th>
                        </tr>
                    </thead>
                    <tbody>
                        {Object.entries(groupedData).map(([category, rows]) => {
                            let categoryQtyTotal = 0;
                            let categoryValueTotal = 0;

                            return (
                                <React.Fragment key={category}>
                                    {/* Category Header */}
                                    <tr className="category-header bg-gray-50">
                                        <td colSpan={5} className="py-2 px-1 font-bold">{category}</td>
                                    </tr>

                                    {/* Category Items */}
                                    {rows.map((row, index) => {
                                        categoryQtyTotal += row.quantity || 0;
                                        categoryValueTotal += row.value || 0;
                                        grandQtyTotal += row.quantity || 0;
                                        grandValueTotal += row.value || 0;

                                        return (
                                            <tr key={`${category}-${index}`} className="hover:bg-gray-50">
                                                <td className="py-1 px-1 font-serif">{row.publisher_nm || ''}</td>
                                                <td className="py-1 px-1 font-serif">{row.author_nm || ''}</td>
                                                <td className="py-1 px-1 font-serif">{row.title || ''}</td>
                                                <td className="py-1 px-1 text-right">{formatInteger(row.quantity)}</td>
                                                <td className="py-1 px-1 text-right">{formatNumber(row.value)}</td>
                                            </tr>
                                        );
                                    })}

                                    {/* Category Total Row */}
                                    <tr className="category-total border-t border-b border-gray-300">
                                        <td colSpan={3} className="py-2 px-1 font-bold">Category Total</td>
                                        <td className="py-2 px-1 text-right font-bold">{formatInteger(categoryQtyTotal)}</td>
                                        <td className="py-2 px-1 text-right font-bold">{formatNumber(categoryValueTotal)}</td>
                                    </tr>
                                </React.Fragment>
                            );
                        })}
                        ...
                        <tr className="grand-total border-t-2 border-b-2 border-gray-400">
                            <td colSpan={3} className="py-2 px-1 font-bold text-sm">Grand Total</td>
                            <td className="py-2 px-1 text-right font-bold text-sm">{formatInteger(grandQtyTotal)}</td>
                            <td className="py-2 px-1 text-right font-bold text-sm">{formatNumber(grandValueTotal)}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
    ...
    return (
        <div className="min-h-screen bg-gray-50 p-4 md:p-6">
            ...
            <PageHeader
                ...
                title="Category Publisher Author Wise Sales"
                subtitle="Generate category, publisher, author wise sales report"
            />
