...
export default function AuthorPublisherSales() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await api.get('/auth/reports/author-publisher-sales/', {
                params: {
                    branch_id: branchId,
    ...
    const handleExportPDF = () => {
      ...
      <html>
      <head>
        <title>Author/Publisher Sales Report</title>
        <style>
          * { box-sizing: border-box; }
          ...
          .author-header { font-weight: bold; }
          .author-total td, .grand-total td { 
            font-weight: bold;
            border-top: 1px solid #b5b3b3;
            border-bottom: 1px solid #b5b3b3;
            padding: 5px 6px;
    ...
    const handleExportExcel = () => {
        ...
        // Add header row
        excelData.push(['Author', 'Publisher', 'Title', 'Quantity', 'Value']);
        ...
        XLSX.utils.book_append_sheet(wb, ws, 'Author Publisher Sales');

        // Generate filename with date range
        const filename = `Author_Publisher_Sales_${formData.date_from}_to_${formData.date_to}.xlsx`;
    ...
    // Render the report table matching JasperReports format with author grouping
    const renderReport = () => {
        ...
        const groupedData = {};
        reportData.forEach(row => {
            const author = row.author_nm || 'Unknown Author';
            if (!groupedData[author]) {
                groupedData[author] = [];
            }
            groupedData[author].push(row);
        });
        ...
        return (
            <div ref={reportRef} className="bg-white">
                ...
                <div className="report-header mb-4">
                    ...
                    <div className="report-title text-base font-bold mt-2">Sale Register (Author / Publisher)</div>
                ...
                <table className="w-full text-xs">
                    <thead>
                        <tr className="border-t border-b border-gray-300">
                            ...
                            <th className="py-2 px-1 text-right w-20">Qty</th>
                            <th className="py-2 px-1 text-right w-24">Value</th>
                        </tr>
                    </thead>
                    <tbody>
                        {Object.entries(groupedData).map(([author, rows]) => {
                            let authorQtyTotal = 0;
                            let authorValueTotal = 0;

                            return (
                                <React.Fragment key={author}>
                                    {/* Author Header */}
                                    <tr className="author-header bg-gray-50">
                                        <td colSpan={4} className="py-2 px-1 font-bold">{author}</td>
                                    </tr>

                                    {/* Author's Items */}
                                    {rows.map((row, index) => {
                                        authorQtyTotal += row.quantity || 0;
                                        authorValueTotal += row.value || 0;
                                        grandQtyTotal += row.quantity || 0;
                                        grandValueTotal += row.value || 0;

                                        return (
                                            <tr key={`${author}-${index}`} className="hover:bg-gray-50">
                                                <td className="py-1 px-1 pl-4 font-serif">{row.publisher_nm || ''}</td>
                                    ...
                                    {/* Author Total Row */}
                                    <tr className="author-total border-t border-b border-gray-300">
                                        <td colSpan={2} className="py-2 px-1 font-bold">Author Total</td>
                                        <td className="py-2 px-1 text-right font-bold">{formatInteger(authorQtyTotal)}</td>
                                        <td className="py-2 px-1 text-right font-bold">{formatNumber(authorValueTotal)}</td>
                                    </tr>
                                </React.Fragment>
                            );
                        })}
    ...
    return (
        <div className="min-h-screen bg-gray-50 p-4 md:p-6">
            ...
            <PageHeader
                ...
                subtitle="Generate author and publisher wise sales report"
            />
