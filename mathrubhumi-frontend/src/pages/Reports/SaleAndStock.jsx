...
export default function SaleAndStock() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await api.get('/auth/reports/sale-and-stock/', {
        params: {
          branch_id: branchId,
  ...
  const handleExportPDF = () => {
    ...
    printWindow.document.write(`
      ...
      <html>
      <head>
        <title>Sale and Stock Report</title>
        <style>
  ...
  const handleExportExcel = () => {
    ...
    // Add header row
    excelData.push(['Publisher', 'Title', 'Sold Quantity', 'Stock']);
    ...
    reportData.forEach(row => {
      ...
      grandTotalSold += row.sold_quantity || 0;
    });

    // Add grand total
    excelData.push(['Grand Total', '', grandTotalSold, '']);
    ...
    ws['!cols'] = [
      { wch: 25 }, // Publisher
      { wch: 50 }, // Title
      { wch: 15 }, // Sold Quantity
      { wch: 12 }, // Stock
    ];

    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Sale and Stock');

    // Generate filename with date range
    const filename = `Sale_and_Stock_${formData.date_from}_to_${formData.date_to}.xlsx`;
    ...
    return (
      <div ref={reportRef} className="bg-white">
        ...
        <div className="report-header mb-4">
          ...
          <div className="report-title text-base font-bold mt-2">Sale and Stock</div>
        ...
        <table className="w-full text-xs">
          <thead>
            <tr className="border-t border-b border-gray-300">
              <th className="py-2 px-1 text-left">Publisher</th>
              <th className="py-2 px-1 text-left">Title</th>
              <th className="py-2 px-1 text-right w-32">Sold Quantity</th>
              <th className="py-2 px-1 text-right w-32">Stock</th>
            </tr>
          </thead>
          <tbody>
            {reportData.map((row, index) => {
              ...
              return (
                <tr key={index} className="hover:bg-gray-50">
                  ...
                  <td className="py-1 px-1 text-right">{formatInteger(row.sold_quantity)}</td>
                  <td className="py-1 px-1 text-right">{formatInteger(row.stock)}</td>
                </tr>
              );
            })}
            ...
            <tr className="grand-total border-t-2 border-b-2 border-gray-400">
              <td colSpan={2} className="py-2 px-1 text-right font-bold text-sm">Grand Total</td>
              <td className="py-2 px-1 text-right font-bold text-sm">{formatInteger(grandTotalSold)}</td>
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 md:p-6">
      ...
      <PageHeader
        ...
        title="Sale and Stock"
        subtitle="Generate sale and stock report"
      />
