...
export default function CialSaleRegister() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await api.get('/auth/reports/cial-sale-register/', {
        params: {
          branch_id: branchId,
  ...
  const handleExportPDF = () => {
    ...
    printWindow.document.write(`
      ...
      <html>
      <head>
        <title>CIAL Sales Register Report</title>
        <style>
  ...
  const handleExportExcel = () => {
    ...
    const excelData = [];
    let grandTotals = { nos: 0, discount: 0, nett: 0 };

    // Add header row
    excelData.push(['Date', 'Nos', 'Discount', 'Nett Sale']);
    ...
    reportData.forEach(row => {
      ...
      grandTotals.nos += row.nos || 0;
      grandTotals.discount += row.discount || 0;
      grandTotals.nett += row.nett_sale || 0;
    });

    // Add grand total
    excelData.push(['Grand Total', grandTotals.nos, grandTotals.discount, grandTotals.nett]);
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'CIAL Sales Register');

    // Generate filename with date range
    const filename = `CIAL_Sales_Register_${formData.date_from}_to_${formData.date_to}.xlsx`;
    ...
    return (
      <div ref={reportRef} className="bg-white">
        ...
        <div className="report-header mb-4">
          ...
          <div className="report-title text-base font-bold mt-2">CIAL Sales Register</div>
        ...
        <table className="w-full text-xs">
          <thead>
            <tr className="border-t border-b border-gray-300">
              <th className="py-2 px-1 text-left w-24">Date</th>
              <th className="py-2 px-1 text-right w-20">Nos</th>
              <th className="py-2 px-1 text-right w-32">Discount</th>
              <th className="py-2 px-1 text-right w-32">Nett Sale</th>
            </tr>
          </thead>
          <tbody>
            {reportData.map((row, index) => {
              ...
              return (
                <tr key={index} className="hover:bg-gray-50">
                  ...
                  <td className="py-1 px-1 text-right">{formatInteger(row.nos)}</td>
                  <td className="py-1 px-1 text-right">{formatNumber(row.discount)}</td>
            ...
            <tr className="grand-total border-t-2 border-b-2 border-gray-400">
              ...
              <td className="py-2 px-1 text-right font-bold text-sm">{formatInteger(grandTotals.nos)}</td>
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 md:p-6">
      ...
      <PageHeader
        ...
        title="CIAL Sale Register"
        subtitle="Generate CIAL sales register report"
      />
