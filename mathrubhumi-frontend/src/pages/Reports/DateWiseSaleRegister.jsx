...
export default function DateWiseSaleRegister() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await api.get('/auth/reports/date-wise-sale-register/', {
        params: {
          branch_id: branchId,
  ...
  const handleExportPDF = () => {
    ...
    printWindow.document.write(`
      ...
      <html>
      <head>
        <title>Date-Wise Sale Register Report</title>
        <style>
  ...
  const handleExportExcel = () => {
    ...
    // Add header row
    excelData.push(['Bill No', 'Type', 'Customer', 'Gross', 'Discount', 'Freight', 'Nett']);
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Date Wise Sale Register');

    // Generate filename with date range
    const filename = `Date_Wise_Sale_Register_${formData.date_from}_to_${formData.date_to}.xlsx`;
    ...
    return (
      <div ref={reportRef} className="bg-white">
        ...
        <div className="report-header mb-4">
          ...
          <div className="report-title text-base font-bold mt-2">Sale Register (Date-wise)</div>
          <div className="report-subtitle text-sm text-gray-600">
            Date-wise Sale Register from {formatDate(reportParams?.date_from)} to {formatDate(reportParams?.date_to)}
          </div>
        </div>
        ...
        <table className="w-full text-xs">
          <thead>
            <tr className="border-t border-b border-gray-300">
              <th className="py-2 px-1 text-left w-20">Bill No</th>
              <th className="py-2 px-1 text-left w-14">Type</th>
              ...
              <th className="py-2 px-1 text-right w-16">Nett</th>
            </tr>
          </thead>
          <tbody>
              ...
              return (
                <React.Fragment key={dateKey}>
                  ...
                  <tr className="date-header">
                    <td colSpan={7} className="py-2 font-bold text-sm">
                      {formatDate(dateKey)}
                    </td>
                  </tr>
                  ...
                  <tr className="daily-total border-t border-b border-gray-300">
                    <td colSpan={3} className="py-2 px-1 font-bold">Daily Total</td>
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 md:p-6">
      ...
      <PageHeader
        ...
        title="Date-Wise Sale Register"
        subtitle="Generate sale register report by date"
      />
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-5">
              ...
              <div>
                ...
                >
                  ...
                  {saleTypes.map((type) => (
                    <option key={type.sale_typeid} value={type.sale_typeid}>
                      {type.sale_type}
                    </option>
                  ))}
                </select>
